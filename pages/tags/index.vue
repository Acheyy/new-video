<template>
  <div class="my-8">
    {{$t('tags')}}:
    <div class="mt-4">
      <template v-for="(category, index) in tagsData" :key="index">
        <NuxtLink :to="localePath('/tags/' + category.slug)" class="btn m-2">
          {{ category.name }}
        </NuxtLink>
      </template>
    </div>
...
useSeoMeta({
  title: `Tags - SKBJ Korean BJ`,
  twitterTitle: `Tags - SKBJ Korean BJ`,
