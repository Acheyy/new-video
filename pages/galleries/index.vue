<template>
  ...
  <div class="cards-wrapper ">
    ...
    <CardLoading
      ...
      v-if="pending"
    ></CardLoading>
    <CardGallery v-if="!pending && !error" v-for="(video, index) in videos.videos" :data="video"></CardGallery>
  </div>
...
const { pending, data: videos, error } = await useLazyFetch(
  () =>
    `https://skbj.tv/api/galleries?limit=30&page=${router.currentRoute.value.query.page}`,
  {
    onResponseError() {
