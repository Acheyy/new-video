<template>
 ...
 v-for="(video, index) in videos.videos" :data="video"></Card>
  ...
  const { pending, data: videos, error } = await useLazyFetch(
    () =>
      `https://kbjfree.tv/api/videos/special?limit=30&page=${router.currentRoute.value.query.page}`,
    {
      onResponseError() {
  ...
  () => {
    useSeoMeta({
      title: `${$t("specialSales")} | Page ${route.query.page} - KBJFree`,
      twitterTitle: `${$t("specialSales")} - KBJFree`,
